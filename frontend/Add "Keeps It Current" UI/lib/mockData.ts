...
export const refundDemoKnowledge = `Refunds are usually allowed within 14 days of purchase. If the item is damaged, ask the customer to send a photo. For refunds above $100, support must ask a manager before approving. If the product was already used, only store credit is allowed unless the manager approves. VIP customers can receive faster processing. The support team should respond politely and include the expected processing time of 3-5 business days.`;

export const refundDemoScenario = `Customer bought the product 10 days ago. The item arrived damaged. Refund amount is $120. Customer is not VIP.`;
...
/**
 * Backend function: generateSkillFile(knowledge, workflowType)
 * Mock implementation — swap for a real backend call when one exists.
 */
export function generateSkillFile(
  knowledge: string,
  workflowType: WorkflowKey
): string {
  const blueprint = skillBlueprints[workflowType];
  const trimmedContext = knowledge.trim();
...
/**
 * Backend function: runSkillOnScenario(skillFile, scenario, workflowType)
 * Mock implementation — swap for a real backend call when one exists.
 * `skillFile` isn't parsed here since the mock reasoning is rule-based,
 * but it's part of the signature so this can be swapped for a real call.
 */
export function runSkillOnScenario(
  skillFile: string,
  scenario: string,
  workflowType: WorkflowKey
): ScenarioDecision {
  ...
  if (workflow === "refund") {
    if (riskHit) {
      return {
        ...
        reasoning: [
          `Detected risk language: "${riskHit}" in the customer message.`,
          "Rule: abusive or legal-threat cases require immediate human handoff — automated replies pause.",
        ],
    ...
    if (amount && amount > 500) {
      return {
        ...
        reasoning: [
          `Requested refund amount is $${amount}.`,
          "Rule: refunds over $500 require Finance sign-off before funds are released.",
        ],
    ...
    if (/damaged|broken|defective|cracked|shattered/.test(text)) {
      return {
        ...
        reasoning: [
          "Scenario describes a damaged or defective item.",
          "Rule: damaged/defective items get a full refund regardless of the 30/90-day window, pending photo evidence.",
        ],
    ...
    return {
      ...
      reasoning: [
        "No damage, risk language, or high-value flag detected.",
        "Default order-age rule applies: refund or store credit based on the 30/90-day window.",
      ],
  ...
  if (workflow === "onboarding") {
    ...
    if (delayed) {
      return {
        ...
        reasoning: [
          "Scenario indicates the laptop or equipment is delayed (e.g. stuck in customs).",
          "Rule: if equipment hasn't shipped or arrived by day 2, escalate to IT-ops on-call.",
        ],
    ...
    if (remote) {
      return {
        ...
        reasoning: [
          "New hire is remote or international.",
          "Rule: remote/international hires require extra coordination with People Ops for payroll and equipment customs.",
        ],
    ...
    return {
      ...
      reasoning: [
        "No delay or remote-hire signals detected.",
        "Default Day 1 onboarding checklist applies.",
      ],
  ...
  if (workflow === "incident") {
    ...
    if (paymentsOrAuth) {
      return {
        ...
        reasoning: [
          "Scenario mentions payments, auth, checkout, or login systems.",
          "Rule: any root cause touching payments or auth requires immediate Security involvement.",
        ],
    ...
    if (highErrorRate) {
      return {
        ...
        reasoning: [
          "Scenario describes a customer-facing outage or rising error rate.",
          "Rule: customer-facing outage or rising error rate meets the P1 bar.",
        ],
    ...
    return {
      ...
      reasoning: [
        "No customer-facing outage or payments/auth signal detected.",
        "Treated as degraded performance with a workaround available.",
      ],
  ...
  if (enterpriseSignal || (size !== null && size >= 500)) {
    return {
      ...
      reasoning: [
        "Scenario signals enterprise scale (size 500+, or mentions SSO/procurement/enterprise).",
        "Rule: enterprise signals route the lead to an Enterprise AE plus Sales Engineering.",
      ],
  ...
  if (size !== null && size < 10) {
    return {
      ...
      reasoning: [
        "Company size is under 10 employees.",
        "Rule: sub-10-employee companies route to the self-serve/PLG funnel, not a rep.",
      ],
  ...
  return {
    ...
    reasoning: [
      "No enterprise or self-serve signal detected.",
      "Default BANT qualification routes the lead to the standard AE queue.",
    ],
...
/**
 * Backend function: updateSkill(currentSkill, updateText, workflowType)
 * Mock implementation — detects a "$X ... instead of $Y" style rule change
 * and appends it to the skill file, falling back to a generic change log
 * entry when no dollar-amount pattern is found.
 */
export function updateSkill(
  ...
  if (!trimmed) {
    return {
      changeLog: ["No update provided — nothing to apply."],
      updatedSkill: currentSkill,
    };
  }

  const amountMatches = trimmed.match(/\$\s?[\d,]+/g);
  const amounts = amountMatches
    ? amountMatches.map((a) => parseInt(a.replace(/[$,\s]/g, ""), 10))
    : [];

  const changeLog: string[] = [];
  if (amounts.length >= 2) {
    const [newValue, oldValue] = /instead of/i.test(trimmed)
      ? [amounts[0], amounts[1]]
      : [amounts[1], amounts[0]];
    changeLog.push(
      `Modified rule: manager approval threshold changed from $${oldValue} to $${newValue}.`
    );
    changeLog.push("Skill file updated to reflect the new escalation rule.");
  } else {
    changeLog.push(
      `Modified rule: applied the new ${workflowOptions.find((o) => o.value === workflowType)?.label ?? workflowType} update to the skill's decision rules.`
    );
    changeLog.push("Skill file updated to reflect the latest company knowledge.");
  }

  const updatedSkill = currentSkill
    ? `${currentSkill}\n## Update Applied (${new Date().toLocaleDateString()})\n${trimmed}\n`
    : currentSkill;

  return { changeLog, updatedSkill };
}
