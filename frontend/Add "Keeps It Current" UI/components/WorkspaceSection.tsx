...
import { Copy, Check, Wand2, Loader2, AlertCircle, Download, Braces } from "lucide-react";
import {
  WorkflowKey,
  workflowOptions,
  sampleKnowledge,
...
export default function WorkspaceSection({
  ...
  const [copiedType, setCopiedType] = useState<"markdown" | "json" | null>(null);

  const handleLoadSample = () => {
    onKnowledgeChange(sampleKnowledge[workflow]);
  };

  const flashCopied = (type: "markdown" | "json") => {
    setCopiedType(type);
    setTimeout(() => setCopiedType((current) => (current === type ? null : current)), 1800);
  };
  ...
  const handleDownloadMd = () => {
    ...
    link.href = url;
    link.download = `${workflow}.skill.md`;
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="flex flex-col rounded-2xl border border-white/10 bg-white/[0.03] p-6">
          ...
          <div className="flex flex-1 flex-col overflow-hidden rounded-lg border border-white/10 bg-[#0b0d12]">
            {/* Code viewer chrome */}
            <div className="flex flex-wrap items-center gap-2 border-b border-white/10 bg-white/[0.03] px-4 py-2.5">
              ...
              <div className="ml-auto flex flex-wrap items-center gap-1.5">
                {copiedType && (
                  <span className="mr-1 flex items-center gap-1 text-xs font-medium text-emerald-400">
                ...
                <button
                  ...
                  className="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-xs font-medium text-slate-300 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40"
                >
